		...
		showFiles : function(data, $container){
			...
			$.ajax({
				...
				complete : function(){
					...
					//根据总图片数及每页大小加载分页
					var total_page = Math.floor(upload_image_num/pagesize);

					for (var page_index = 1;page_index <= total_page;page_index++){
						...
						$.ajax({
							...
							data: {size:pagesize,start:page_index*pagesize},
